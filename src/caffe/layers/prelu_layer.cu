...
__global__ void PReLUForward(
    const int n, const int channels, const int hw,
...
__global__ void PReLUParamBackward(const int n, const Dtype *in_diff,
    ...
    CUDA_KERNEL_LOOP(index, n) {
        out_diff[index] =
            in_diff[index] * in_data[index] * (in_data[index] <= 0);
    }
}
...
void PReLULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*> &bottom,
    ...
    if (top[0] == bottom[0]) {
        caffe_gpu_memcpy(
            count, bottom_data, bottom_memory_.mutable_gpu_data());
    }
...
void PReLULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*> &top,
    ...
    if (this->param_propagate_down_[0]) {
        ...
        for (int n = 0; n < bottom[0]->num(); ++n) {
            ...
            if (channel_shared_) {
                Dtype d;
                caffe_gpu_dot<Dtype>(
                    channels * hw, multiplier_.gpu_diff(),
                    multiplier_.gpu_data(), &d);
                dsum += d;
            } else {
                caffe_gpu_gemv<Dtype>(
                    CblasNoTrans, channels, hw, 1.,
    ...
    if (propagate_down[0]) {
        ...
        PReLUBackward<Dtype>
        <<< CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
            count, channels, hw, top_diff, bottom_data,
